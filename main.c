...
int32_t array_upor (int32_t* array_ptr, int32_t* auto_ptr, int32_t* zero_index);
...
extern int32_t array_zapis (int32_t* array_ptr, size_t array_size);
 ...
 while (number_serial<array_size)
   {
    ...
    array_upor(new_array, array_ptr,znach0);
	return new_array;
}

int32_t array_upor (int32_t* new_array, int32_t* source_mass ,int32_t* zero_index)
{
	...
	while (a < (zero_index))  
	{
        *(new_array+i)=*(source_mass+i);  // записываем значения с индексом a в новый массив
        i++;
        a=source_mass+i;
}
}
